        ...
        miembros.forEach(miembro => {
            ...
            card.innerHTML = `
                ...
                <div class="flex gap-3 mt-4">
                    <button class="btn-edit flex-1 bg-white text-black py-2 rounded text-xs font-semibold hover:opacity-80 transition" onclick="prepararEdicion(${miembro.id})">
                        Editar
                    </button>
                    <button class="btn-delete flex-1 bg-transparent border border-white text-white py-2 rounded text-xs font-semibold hover:bg-red-600 hover:border-red-600 transition" onclick="eliminarMiembro(${miembro.id})">
                        Eliminar
                    </button>
                </div>
            `;
                ...
                <label class="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-3 cursor-pointer hover:bg-white/10 hover:border-white/30 transition-all group">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex gap-2 mt-1">
                            <span class="text-xs px-2 py-0.5 bg-white/10 text-gray-300 rounded-full border border-white/10">
        ...
        eventos.forEach(evento => {
            ...
            const participantesHTML = evento.participantes.map(id => {
                ...
                if (miembro) {
                    ...
                    return `
                        <span class="bg-white/10 text-white border border-white/20 px-3 py-1 rounded-full text-xs font-semibold hover:bg-white/20 transition">
                            ${nombreCompleto}
                        </span>
                    `;
                }
                ...
                return `
                    <span class="bg-white/10 text-gray-300 border border-white/10 px-3 py-1 rounded-full text-xs">
                        ID ${id}
                    </span>
                `;
            }).join('');
            ...
            card.innerHTML = `
                <div class="flex justify-between items-start gap-3 mb-4">
                    <div class="flex-1">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <h3 class="text-xl font-bold text-white group-hover:text-white transition">
                ...
                <div class="flex gap-2 pt-4 border-t border-white/10">
                    <button onclick="editarEvento(${evento.id})" class="flex-1 bg-white text-black font-semibold px-3 py-2 rounded-lg transition hover:opacity-80 text-sm">
                        Editar
                    </button>
...
window.editarEvento = async (id) => {
    try {
        ...
        document.getElementById('formEvento').scrollIntoView({
            behavior: 'smooth',
            block: 'center'
        });
...
window.eliminarEvento = async (id) => {
    const confirmDialog = confirm('¿Estás seguro de eliminar este evento/proyecto?\n\nEsta acción no se puede deshacer.');
